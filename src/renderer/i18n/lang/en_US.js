export default {
  binding: 'Binding Telegram:',
  addError: 'Add error',
  yes: 'yes',
  no: 'no',
  submit: 'Submit now',
  meneTotal: 'The total number of messages in the menu bar',
  userEntry: 'User entry prompt',
  userMessageNum: 'Number of user messages received',
  menuMessageNum: 'Total number of messages in the menu bar',
  waitNum: 'Waiting for access in the menu bar',
  voicePrompt: 'Voice prompt',
  tray: 'Tray flashes',
  task: 'Taskbar flashes',
  transfer_task: 'Transfer reminder settings:',
  toBeConnected: 'Pending access prompt settings:',
  groupPrompt: 'Group prompt settings:',
  userPrompt: 'User prompt settings:',
  messagePrompt: 'Message reminder settings:',
  telegramAccount: 'Telegram connection settings',
  accountStart: 'Need to use telegram account to send message 【',
  accountEnd: '】 to aliuge\\',
  correctCode: 'Please enter the correct merchant identification code!',
  listViewImg: `[Image]`,
  listViewFile: `[File]`,
  listViewVoice: `[Voice]`,
  listViewEmoji: '[Emoji]',
  day: 'Day',
  hour: 'Hour',
  minute: 'Minute',
  permanent: 'Permanent',
  pleaseSelect: 'please choose',
  pleaseInput: 'please enter',
  time: 'Time',
  theReason: 'the reason',
  addBlackStartTime: 'Start to pull black time',
  addBlackEndTime: 'End pull black time',
  addBlackTime: 'Pull Black time:',
  deleteSuccess: 'successfully deleted!',
  updateNickName: 'Change username',
  inputName: 'Please enter a new nickname!',
  getOutApp: 'This operation key exits the program, do you want to continue?',
  textToSpeech: 'Text-to-speech',
  playErr: 'Play failed!',
  tts: 'Play text',
  sendNewMessage: "Sent you a new message",
  rogerThat: 'Receive a new group message!',
  kickTip: 'Are you sure you want to kick the selected person out of the group chat?',
  removeBlack: 'Are you sure you want to delete the selected person from the blacklist?',
  addBlack: 'Are you sure you want to add the selected person to the blacklist?',
  dismiss: 'Are you sure you want to unblock the selected person from speaking?',
  disableSendMsg: 'Are you sure you want to prohibit the selected person from speaking?',
  Update: 'The new version is being updated, please wait...',
  updateFailed: 'Update failed',
  loading: 'loading...',
  noMore: 'No more...',
  overLimit: 'The content of the sent message exceeds the sending length',
  downloadFile: 'Download successfully',
  downloadFilErr: 'download failed',
  addGroupMember: 'Please select a group member to operate!',
  selectGroupMember: 'Please select a group member to operate!',
  sendErr: 'Sending failed, please resend!',
  selectImg: 'Please select the picture correctly!',
  NewNews: 'New message received',
  transferSuccess: 'Successful transfer',
  fileExpired: 'The file has expired!',
  noVoice: 'The current environment does not support voice',
  timeShort: 'Talk time is too short',
  permission: 'No recording permission!',
  sendAir: 'Send content cannot be empty!',
  online: 'Online',
  offline: 'Offline',
  goAway: 'Go away',
  refresh: 'Refresh',
  setting: 'Setting',
  outLogin: 'Login out',
  determine: 'Determine',
  cancel: 'Cancel',
  refuse: 'Refuse',
  prompt: 'Prompt',
  loginPrompt: 'Are you sure you want to log out?',
  noCurrent: 'No sessions...',
  notification: 'Transfer notification',
  noGroup: 'No group chat, please contact the business to create a group',
  receive: 'To receive',
  recording: 'Talking in recording...',
  putAway: "Put away",
  unfold: 'Unfold',
  update: 'Confirm to modify to ',
  status: 'status?',
  confirmReception: 'Confirm reception ',
  confirmCancel: "Are you sure you want to end the conversation with ",
  conversation: '？',
  download: 'Click to download file',
  updateSuccess: 'Successfully modified',
  updateError: 'fail to edit',
  connectError: 'System slipped',
  transferState: {
    noSservice: 'Customer service is not currently available,',
    to: 'to',
    confirm: "Are you sure to transfer ",
    question: '?'
  },
  topTitle: {
    currentChat: 'Current session',
    awaitChat: 'Pending session',
    groupChat: 'Group chat management',
    leaveMessage: 'Message List',
    chatLog: 'Session record'
  },
  currentInfo: {
    userInfo: 'User Info',
    quickReply: 'Quick Reply',
    accessInformation: 'Access information',
    otherOperations: 'Other operations',
    pleaseEnterKeyword: 'Please enter the question keyword',
    noRelatedQuestions: 'No related questions...',
    ip: 'Ip',
    area: 'Area',
    transferCurrentConversation: 'Transfer current conversation',
    transferSession: 'Transfer session',
    closeCurrentSession: 'Close current session',
    closeSession: 'Close session',
    expression: 'Expression',
    image: 'Image',
    file: 'File',
    voice: 'Voice',
    send: 'Send',
    wrap: 'Wrap',
    pleaseEnter: 'Please enter...',
    transfer: 'Waiting for transfer'
  },
  awaitInfo: {
    userName: 'Visitor name',
    searchTime: 'Time',
    startTime: 'Starting time',
    endTime: 'End Time',
    title: 'List of waiting users',
    search: 'please enter user name',
    tableTitle: {
      visitorAvatar: 'Visitor avatar',
      visitorName: 'Visitor name',
      visitorIp: 'Visitor ip',
      interviewTime: 'Interview time',
      visitorType: 'User level',
      operating: 'Operating',
      Tourist: 'Tourist',
      user: 'User',
      reception: 'Reception'
    },
    sessionRecord: {
      visitorAvatar: 'Visitor avatar',
      visitorName: 'Visitor name',
      visitorIp: 'Visitor ip',
      visitorArea: 'Area',
      startReceptionTime: 'Start reception time',
      endReceptionTime: 'End of reception time',
      receptionTime: 'Reception time',
      viewLog: 'View chat history'
    }
  },
  groupInfo: {
    groupMember: 'Group member',
    admin: '(administrator)',
    banned: 'Banned',
    blocked: 'Blocked',
    mute: 'Mute',
    isMute: 'No talking',
    unmute: 'Unmute',
    addBlack: 'Add to blacklist',
    removeBlack: 'Remove blacklist',
    kickOut: 'Kick out group chat',
    pull: 'Pull people into the group',
    AddGroupMembers: 'Add group members',
    noGroupMembers: 'No group members...',
    addBlackRemark: 'Reasons for joining the blacklist',
    reason: 'Please enter the reason for being blacklisted'
  },
  login: {
    shopUrl: 'Business Url',
    shopCode: 'Merchant ID',
    pleaseShopCode: 'Please enter the business ID',
    account: 'Account',
    pleaseAccount: 'Please enter customer service account',
    password: 'Password',
    pleasePassword: 'Please enter password',
    logIn: 'Log in',
    remember: 'Remember account/password',
  },
  transfer: {
    avatar: 'Avatar',
    nickname: 'Nickname',
    transferNumber: 'Number of transfers',
    transfer: 'Transfer'
  },
  message: {
    name: 'username',
    number: 'mobile phone number',
    messageContent: 'Message content',
    email: 'Email',
    messageTime: 'Message Time',
    status: 'Current state',
    operating: 'Operating',
    edit: 'Edit',
    editSuccess: 'Processed successfully',
    remark: 'Remark',
    editRemark: 'Please enter a comment',
    untreated: 'Untreated',
    processed: 'Processed',
    notReached: 'Can not be reached',
    processingTime: 'Processing time',
    selectStatus: 'Please select processing status!'
  }
}

